...
namespace gpu_directx
{
   ...
   LRESULT CALLBACK Draw2DirectXWndProc(HWND hwnd, UINT message, WPARAM wparam, LPARAM lparam)
   {
   ...
   void approach::initialize_gpu_approach()
   {
      ...
      // Select physical device to be used for the DirectX example
      ...
      //            if (index > gpuCount - 1) {
      //               std::cerr << "Selected device index " << index << " is out of range, reverting to device 0 (use -listgpus to show available DirectX devices)" << "\n";
      //            }
      //            else {
      ...
      //            debug() << "Available DirectX devices" << "\n";
      ...
      //m_pgpudevice = new vks::DirectXDevice(physicalDevice);

      //// Derived examples can enable extensions based on the list of supported extensions read from the physical device
      //getEnabledExtensions();

      //result = directxDevice->createLogicalDevice(enabledFeatures, enabledDeviceExtensions, deviceCreatepNextChain);
      //if (result != VK_SUCCESS) {
      ...
      //vkGetDeviceQueue(device, directxDevice->queueFamilyIndices.graphics, 0, &queue);
   ...
   //   {
   //
   //      ::pointer < ::gpu_directx::context > pgpucontext;
   //
   //#ifdef WINDOWS_DESKTOP
   //
