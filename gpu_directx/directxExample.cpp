/*
* DirectX Example - Offscreen rendering using a separate framebuffer
*
...
#include "directxexamplebase.h"
#include "DirectXglTFModel.h"

// Offscreen frame buffer properties
#define FB_DIM 512
#define FB_COLOR_FORMAT VK_FORMAT_R8G8B8A8_UNORM

class DirectXExample : public DirectXExampleBase
{
public:
   ...
   DirectXExample() : DirectXExampleBase()
   {
   ...
   void loadAssets()
   {
      ...
      models.plane.loadFromFile(getAssetPath() + "models/plane.gltf", directxDevice, queue, glTFLoadingFlags);
      models.example.loadFromFile(getAssetPath() + "models/chinesedragon.gltf", directxDevice, queue, glTFLoadingFlags);
   }
   ...
   void prepareUniformBuffers()
   {
      ...
      VK_CHECK_RESULT(directxDevice->createBuffer(VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, &uniformBuffers.vsShared, sizeof(UniformData)));
      // Mirror plane vertex shader uniform buffer block
      VK_CHECK_RESULT(directxDevice->createBuffer(VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, &uniformBuffers.vsMirror, sizeof(UniformData)));
      // Offscreen vertex shader uniform buffer block
      VK_CHECK_RESULT(directxDevice->createBuffer(VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, &uniformBuffers.vsOffScreen, sizeof(UniformData)));
      // Map persistent
   ...
   void draw()
   {
      DirectXExampleBase::prepareFrame();
      submitInfo.commandBufferCount = 1;
      submitInfo.pCommandBuffers = &drawCmdBuffers[currentBuffer];
      VK_CHECK_RESULT(vkQueueSubmit(queue, 1, &submitInfo, VK_NULL_HANDLE));
      DirectXExampleBase::submitFrame();
   }
