...
shared_memory::shared_memory(memory_container * pcontainer, void * pMemory, memsize dwSize)
{
   ...
   ::memory_copy(m_memory.m_pbStorage, pMemory, (size_t)dwSize);
...
unsigned char * shared_memory::detach_shared_memory(HGLOBAL & hglobal)
{
   ...
   unsigned char * pbStorage = m_memory.m_pbStorage;
...
void shared_memory::SetHandle(HGLOBAL hGlobalMemory, bool bAllowGrow)
{
   ...
   m_memory.m_pbStorage = (unsigned char *)::GlobalLock(m_hGlobalMemory);
